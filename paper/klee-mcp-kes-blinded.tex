...
\author[a]{Anonymous Author(s)\corref{cor1}}

\address[a]{Affiliation withheld for double-blind review}
...
\end{keyword}
\cortext[cor1]{Corresponding author email withheld for double-blind review.}
\end{frontmatter}
...
An MCP server is a practical interface for exposing symbolic execution to an LLM auditor, and the combination validates and reproduces real-library vulnerabilities and contract violations while cleanly separating confirmed bugs from rescued false positives. The methodological insight worth generalising beyond KLEE is that LLM-supplied bounds encode \emph{caller contracts}, and a mechanical auto-relax retry catches when the contract is wrong. The server, skill, benchmark, and all 14 candidates will be released upon acceptance.
