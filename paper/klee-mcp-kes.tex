...
\usepackage{ecrc}
%% Suppress the in-abstract-block copyright that ecrc.sty inserts by
%% default; the footer copyright on page 1 is retained. This removes
%% the visible duplication near the bottom of page 1 without altering
%% the standard Procedia footer-line copyright notice.
\makeatletter
\def\@CopyrightLine{}
\makeatother
...
\begin{abstract}
Large language models flag plausible vulnerability candidates in C/C++ code but cannot produce concrete crashing witnesses; symbolic execution engines like KLEE can produce such witnesses but need a harness and the right preconditions. We close this loop with \textsc{klee-mcp}, a stateless Model Context Protocol (MCP) server that exposes KLEE as five tools to an LLM auditor, plus a Claude Code skill that drives the workflow from a single natural-language instruction. The auditor emits structured candidates (CWE, tainted inputs, semantic bounds); the server auto-builds a KLEE harness, optionally links whole-library LLVM bitcode, either confirms a candidate with a byte-accurate concrete input or reports it infeasible under the supplied bounds, and emits a disclosure-ready reproducer plus a heuristic exploitability classification. On a small proof-of-concept benchmark of 14 hand-constructed candidates (toy CWE programs plus selected libpng 1.6.47 functions through a whole-library 2.2\,MB bitcode), the pipeline returns the expected verdict on every row; we read this as evidence of internal consistency, not as a precision claim against an external corpus. It also surfaces an unchecked precondition in \texttt{png\_format\_number} (a defense-in-depth contract violation, not internally reachable) and reproduces CVE-2015-8540 in \texttt{png\_check\_keyword} across libpng 1.2.56, 1.6.19, and 1.6.47, with a counterfactual LLM bound that flips the verdict to infeasible. We argue that LLM-supplied semantic bounds encode the caller contract under which safety is being verified, not a performance knob, and demonstrate a self-correcting auto-relax retry when those bounds are wrong
\end{abstract}
...
\paragraph{Methodological contributions} The contribution of this work is not the integration of an LLM with KLEE as such, but three concrete methodological choices that the integration makes possible. (i) A \emph{candidate schema} (CWE, tainted inputs, LLM-supplied semantic bounds, retry policy) that forces the language model to commit to a structured, machine-checkable specification \emph{before} the symbolic engine runs, in place of free-form harness source. (ii) A \emph{retractable-bounds protocol}: LLM-declared preconditions are delivered to KLEE as \texttt{klee\_assume} predicates and automatically relaxed by the server on an infeasible verdict, so that mis-modelled preconditions are surfaced rather than silently mis-verifying. (iii) A \emph{symex-witness-to-disclosure} pipeline that turns each confirmed verdict into a compilable standalone \texttt{reproduce.c}, a \texttt{poc.bin}, and a heuristic exploitability classification (primitive, severity, region, CVSS-3.1 vector) aimed at a coordinated-disclosure workflow.

\paragraph{System and evaluation} We realise these in \textbf{\textsc{klee-mcp}},\footnote{Source, skill, and reproducible benchmark: \url{https://github.com/FHNW-Security-Lab/klee-mcp}} a stateless MCP server exposing KLEE as five tools and a Claude Code skill that drives the full triage pipeline. As a proof-of-concept evaluation we run 14 hand-constructed candidates spanning toy CWE programs, bounded/unbounded sensitivity pairs, and libpng~1.6.47 as a whole-library 2.2\,MB LLVM bitcode; the pipeline produces the expected verdict on all 14 rows on this small benchmark. We further reproduce CVE-2015-8540 in \texttt{png\_check\_keyword}, confirming the same OOB in libpng 1.6.19 and 1.6.47 from the concrete byte-level trigger KLEE synthesises, and use a baseline contrast with raw KLEE (no LLM bounds) to quantify how much of the speedup is attributable to the candidate-schema discipline.
...
The classifier also extracts (i) the target memory region (\texttt{stack} / \texttt{heap} / \texttt{static}) from KLEE's allocation note (\texttt{MO... allocated at ...: \%x = alloca i32} vs \texttt{malloc}), with a CWE-driven fallback, and (ii) a \emph{program-counter-overwrite possibility} flag that is \texttt{true} for arbitrary writes and for stack-located bounded writes. A heuristic CVSS-3.1 base vector is attached purely as a disclosure-template placeholder (filled under a default network-attacker assumption); it is not a severity claim and is intended to be replaced by a human triager before any external use.

We emphasise that the classifier is a \emph{triage signal} rather than ground truth: the labels are inferred from KLEE's faulting-address expression, declared CWE, and a one-line source heuristic, and they do not constitute a working exploit. To check the rule set against human judgement on the cases we already have, we cross-checked the classifier's label against a manually-assigned label for every confirmed verdict in our benchmark (10 in total across Tables~\ref{tab:results} and~\ref{tab:cve}) by inspecting the offending source line and the KLEE info block; the two labels agreed on all 10 cases. We treat this as evidence that the rule set is internally consistent, not as a precision figure for the classifier in the wild --- a larger evaluation against an externally labelled corpus is left for follow-on work, and the CVSS strings should be reviewed by a human triager before any external use.
...
Table~\ref{tab:results} lists all 14 candidates. Ground-truth verdicts are encoded in each candidate JSON (\texttt{expected\_verdict}). The benchmark covers four phases: (1) toy programs exercising each target CWE, (2) LLM bounds + auto-relax, (3) extracted real libpng functions (self-contained TU), and (4) whole-library libpng+zlib bitcode. A separate reachability micro-benchmark (entry$\to$target probes on a dispatcher example, 2/2 verdicts correct) is shipped with the repo but omitted from Table~\ref{tab:results} for space.

\begin{table}[t]
\caption{Hand-constructed 14-candidate proof-of-concept benchmark. B\,=\,\texttt{use\_bounds=true}; R\,=\,server auto-relaxed after a bounded infeasible run. The pipeline returns the expected verdict on every row of this small benchmark; the result is descriptive of these cases, not a precision claim against an external corpus.}
\label{tab:results}
\scriptsize
...
\bottomrule
\end{tabular*}
{\scriptsize $^\dagger$ Contract-violation finding, not internally reachable in libpng 1.6.47; discussed under Row~14 below.}
\end{table}

\paragraph{Result on the proof-of-concept benchmark} The pipeline produces the expected verdict on all 14 hand-constructed rows; this is descriptive of the small benchmark, not a precision figure against an external corpus. Median wall time 2.4\,s; worst case 42.9\,s on the whole-library \texttt{png\_check\_fp\_number} exhaustion. Memory usage stayed well below the 16\,GB cap on every row. Phase-1 and -2 rows complete sub-second; extracted libpng targets (P3a) take 6--31\,s; whole-library linked targets (P3b) 4--43\,s.
...
\paragraph{Baseline contrast: with vs without LLM bounds} We treat raw KLEE (the same harness without \texttt{klee\_assume} predicates) as the natural within-tool baseline and ran the same five libpng candidates twice --- once with the LLM-supplied bounds and once without. Table~\ref{tab:baseline} reports the pair.

\begin{table}[t]
\caption{Bounded vs unbounded runs on five libpng candidates. The bounded column reproduces rows from Table~\ref{tab:results}; the unbounded column reruns the same harness with \texttt{use\_bounds=false}. ``conf.'' = confirmed, ``inf.'' = infeasible. Path counts are KLEE's \emph{completed} / \emph{partially completed} totals at run end.}
\label{tab:baseline}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular*}{\hsize}{@{\extracolsep{\fill}}lrlrl@{}}
\toprule
\textbf{Candidate} & \textbf{Bounded wall} & \textbf{Bounded verdict} & \textbf{Unbounded wall} & \textbf{Unbounded verdict} \\
\midrule
libpng\_sig\_cmp        &  5.1\,s & inf.\ (56/0)     &  5.0\,s & inf.\ (56/0) \\
libpng\_fp\_number      & 30.9\,s & inf.\ (1438/0)   &  9.4\,s & conf.\ (0/227), arbitrary\_read \\
libpng\_fp\_whole       & 42.9\,s & inf.\ (1438/0)   & 36.8\,s & conf.\ (0/790), arbitrary\_read \\
libpng\_push\_read      &  4.1\,s & inf.\ (9/36)     &  3.8\,s & conf.\ (1/56), bounded\_read \\
libpng\_format\_number  &  4.2\,s & conf.\ (0/2)     &  3.7\,s & conf.\ (0/3), arbitrary\_write \\
\bottomrule
\end{tabular*}
\end{table}

Two readings of the same table. \emph{Baseline reading (R2-1):} the bounded runs yield definitive \texttt{infeasible} proofs (bounded path-exhaustion) on three of the four functions that are safe under their documented caller contract, plus a confirmed bug on the function whose contract is structurally weak (\texttt{format\_number}). Removing the bounds replaces those proofs with crashing witnesses on every row whose function trusts a size argument blindly. The bounds are therefore the discipline that converts a witness-only mode (raw KLEE) into a witness-or-proof mode (klee-mcp). A note on the wall-times: the bounded runs are sometimes \emph{slower} than the unbounded ones because the two columns measure different work --- the bounded \texttt{infeasible} verdicts come from full path exhaustion within the bound (e.g., 1438 complete paths for \texttt{fp\_number}), while the unbounded \texttt{confirmed} verdicts stop at the first crashing path KLEE encounters via \texttt{--exit-on-error-type} (227 partial paths and 0 completed paths for the same target). The relevant comparison for R2-1 is therefore not raw wall-time but \emph{what each mode produces in that time}: an exhaustive proof vs.\ a first-witness early exit. \emph{Empirical reading:} three structurally different libpng functions (\texttt{png\_check\_fp\_number}, \texttt{png\_format\_number}, \texttt{png\_push\_fill\_buffer} inside the push parser reached via \texttt{libpng\_push\_read}) show the same caller-contract OOB class --- a size or pointer argument trusted by the function without an internal size check --- while \texttt{png\_sig\_cmp} is robust because it clamps its inputs internally. We do not claim these as new CVEs; they fall in the same defense-in-depth class as \texttt{png\_format\_number} (the call sites we inspected respect the contract), but the table demonstrates that the pipeline systematically surfaces the class wherever it occurs.

We do not present a baseline against external tools (cppcheck, clang-analyzer, libFuzzer, oss-fuzz-gen) here. A fair cross-tool comparison requires a different experimental design --- the units those tools consume (rule sets, fuzz drivers, corpora) are not commensurate with our structured candidate schema, and matching them on equal terms is itself a research question. We mark a cross-tool study as the obvious follow-on.

\paragraph{Historical CVE reproduction: CVE-2015-8540 \cite{cve20158540}} We extracted \texttt{png\_check\_keyword} from libpng~1.6.19 (last release before the 1.6.20 fix) into a byte-identical standalone TU, driven with a 4-byte symbolic non-null-terminated \texttt{key} and an 80-byte output buffer. KLEE \textbf{confirmed the OOB read at line~43 in 1.26\,s}, synthesising \texttt{key = 0x01\,0x01\,0x01\,0x01}: the \texttt{while (*key \&\& key\_len < 79)} loop reads \texttt{key[4]}, one byte past the allocation. Extracting the same function from libpng~1.6.47 (body preserved verbatim, moved to pngset.c) and running the identical harness yields \textbf{the same OOB at line~34 in 1.26\,s with the identical trigger}; the 1.6.20 fix lived in callers (null-terminating before invocation), not inside \texttt{png\_check\_keyword}. To probe whether the bug is layout-specific or structural, we also extracted the 1.2.56 version (which uses a different code path with \texttt{png\_strlen} plus a \texttt{for(*kp)} loop instead of the inline \texttt{while}); KLEE \textbf{confirmed the same class of OOB read at line~27 in 1.16\,s with the same trigger}, demonstrating that the same caller-contract assumption persists across structurally different implementations in the 1.2 and 1.6 series. Adding the counterfactual LLM bound \texttt{key[3] == 0} (modelling the caller contract) flips the verdict on the 1.6.19 harness to \textbf{infeasible in 1.17\,s after exhausting 103 completed paths}, directly illustrating on a real CVE that LLM bounds encode caller contracts and that the same function is safe-under-contract and unsafe-off-contract. Table~\ref{tab:cve} summarises the four runs.

\begin{table}[t]
\caption{CVE-2015-8540 reproductions and counterfactual. Same 4-byte symbolic non-null-terminated \texttt{key}, same 80-byte output buffer in every run.}
\label{tab:cve}
\begin{tabular*}{\hsize}{@{\extracolsep{\fill}}llrl@{}}
\toprule
\textbf{libpng version} & \textbf{Bound applied} & \textbf{Wall} & \textbf{Verdict} \\
\midrule
1.2.56 (\texttt{pngset.c} layout)        & ---                       & 1.16\,s & confirmed (OOB read line 27) \\
1.6.19 (last pre-fix)                    & ---                       & 1.26\,s & confirmed (OOB read line 43) \\
1.6.47 (current, body verbatim)          & ---                       & 1.26\,s & confirmed (OOB read line 34) \\
1.6.19, with counterfactual              & \texttt{key[3] == 0}      & 1.17\,s & infeasible (103 paths exhausted) \\
\bottomrule
\end{tabular*}
\end{table}
...
\textsc{klee-mcp} is fast when LLM bounds are reasonable and the target function has a small enough symbolic surface. Without bounds, real-library parsers time out; we consider this an intended part of the workflow, because semantic bounds are a useful form of domain knowledge that an LLM auditor can propose and that the auto-relax mechanism can check. Whole-library bitcode handles libraries in the libpng / libxml2 class; scaling to codebases with dynamic linking, heavy C++ templates, or inline assembly (libjpeg-turbo's SIMD paths) requires engineering we do not yet report. The heuristic exploitability classifier does \emph{not} prove a working exploit --- it reports the primitive class the crash exposes. Our non-synthetic findings split into two kinds: three byte-level reproductions of CVE-2015-8540 across structurally different libpng versions (Table~\ref{tab:cve}), and one defense-in-depth contract violation in \texttt{png\_format\_number} (Row~14) that is not internally reachable in libpng~1.6.47. For both we were unable to obtain maintainer feedback before the submission deadline; the paper's claim is therefore about the pipeline's \emph{method} and its ability to reproduce known CVEs and surface contract-violation patterns, not about a body of externally-acknowledged \emph{new} vulnerabilities. Our cross-library audit (libpng-tRNS focused, libjpeg-turbo, libxml2) showed the pipeline scales end-to-end on a second whole-library target (libxml2, 11 exhaustively-completed parser paths in 23\,s) but that stock KLEE's \texttt{setjmp}/\texttt{longjmp} handling and state compaction become the binding constraint on deeper image-format targets. Verifying program-counter overwrite, adding \texttt{setjmp}-aware symbolic execution, and running historical CVEs are the obvious follow-ons.

An MCP server is a practical interface for exposing symbolic execution to an LLM auditor, and the combination supports validation and reproduction of selected real-library vulnerability candidates and contract violations while separating confirmed candidates from rescued false positives on the cases examined. The methodological insight worth generalising beyond KLEE is that LLM-supplied bounds encode \emph{caller contracts}, and a mechanical auto-relax retry catches when the contract is wrong. We release the server, skill, benchmark, and all 14 candidates at \url{https://github.com/FHNW-Security-Lab/klee-mcp}.
...
\clearpage
\section*{Appendix~A. Commentary on Reviewer-Requested Revisions}

We thank the reviewers for their constructive feedback. In response to Review~2, we revised the paper in five main ways. \emph{First}, we added a within-tool baseline comparison against raw KLEE without LLM-supplied bounds in Section~\ref{sec:eval}. \emph{Second}, we clarified in Section~\ref{sec:exploit} that the exploitability classifier and CVSS vector are heuristic triage aids rather than validated severity claims, and added a manual cross-check of the classifier's labels against human-assigned labels on all 10 confirmed cases in our benchmark. \emph{Third}, we moderated the real-world-impact claims by framing \texttt{png\_format\_number} as a defense-in-depth contract violation and by reporting CVE-2015-8540 \cite{cve20158540} reproductions across libpng 1.2.56, 1.6.19, and 1.6.47 (Table~\ref{tab:cve}). \emph{Fourth}, we rewrote the contribution framing in Section~\ref{sec:intro} to emphasise the candidate schema, the retractable-bounds protocol, and the symex-witness-to-disclosure pipeline rather than mere system integration. \emph{Fifth}, we revised the abstract, evaluation, and limitations to state that the 14-case benchmark is a small proof-of-concept and not a precision claim against an external corpus. Review~1 raised no required revision; we tightened the framing of Section~\ref{sec:intro} so that the methodological contribution stands out earlier in the paper.
