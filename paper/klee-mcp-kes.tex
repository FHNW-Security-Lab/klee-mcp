...
\paragraph{System and evaluation} We realise these in \textbf{\textsc{klee-mcp}},\footnote{Source, skill, and reproducible benchmark: \url{https://github.com/FHNW-Security-Lab/klee-mcp}} a stateless MCP server exposing KLEE as five tools and a Claude Code skill that drives the full triage pipeline. As a proof-of-concept evaluation we run 14 candidates spanning toy CWE programs, bounded/unbounded sensitivity pairs, and libpng~1.6.47 as a whole-library 2.2\,MB LLVM bitcode; 14/14 verdicts agree with ground truth. We further reproduce CVE-2015-8540 in \texttt{png\_check\_keyword}, confirming the same OOB in libpng 1.6.19 and 1.6.47 from the concrete byte-level trigger KLEE synthesises, and use a baseline contrast with raw KLEE (no LLM bounds) to quantify how much of the speedup is attributable to the candidate-schema discipline.
...
We emphasise that the classifier is a \emph{triage signal} rather than ground truth: the labels are inferred from KLEE's faulting-address expression, declared CWE, and a one-line source heuristic, and they do not constitute a working exploit. As a sanity check we manually labelled the primitive class for every confirmed verdict in our benchmark (10 in total across Tables~\ref{tab:results} and~\ref{tab:cve}) by inspecting the offending source line and the KLEE info block; the classifier's label agreed with the manual label in all 10 cases. We treat this as evidence that the rule set is consistent with itself, not as a precision figure for the classifier in the wild --- a larger evaluation against a labelled corpus is left for follow-on work, and the CVSS strings should be reviewed by a human triager before any external use.
...
%% (Bibliography continues below; see also the response-to-reviewers
...
\clearpage
\section*{Appendix~A: Response to reviewers (revision summary)}

We thank the reviewers for their constructive feedback. Below we list every concrete remark from the original reviews and how the revised manuscript addresses it.

\paragraph{Review~1} The reviewer raised no required revision. We have tightened the framing of Section~\ref{sec:intro} so that the methodological contribution stands out earlier in the paper.

\paragraph{Review~2: ``No strong baselines or comparative evaluation against existing tools''} We added an explicit \emph{Baseline contrast} paragraph in Section~\ref{sec:eval} that quantifies the gap between raw KLEE (no LLM bounds) and \textsc{klee-mcp} on the same target. On \texttt{libpng\_check\_fp\_number}, raw KLEE times out at 60\,s with 1700 partial paths and no verdict, while the LLM-bounded run exhausts 1438 complete paths in 30.9\,s and returns a definitive \texttt{infeasible} proof. The sensitivity pair already in Table~\ref{tab:results} (\texttt{libpng\_fp\_number} bounded vs unbounded) is the within-tool baseline against KLEE itself. We deliberately do not introduce a cross-tool baseline against static analysers or fuzzers because the experimental units they consume (rule sets, fuzz drivers, corpora) are not commensurate with our structured candidate schema; we now state this explicitly in the same paragraph and flag the cross-tool study as the right design for follow-on work.

\paragraph{Review~2: ``Exploitability classification and CVSS scoring are heuristic and not rigorously validated''} We added a paragraph at the end of Section~\ref{sec:exploit} that (i) labels the classifier explicitly as a \emph{triage signal} rather than ground truth, (ii) reports a manual cross-check of the classifier's primitive label against a human-assigned label for every confirmed verdict in our benchmark (10/10 agreed), (iii) flags this as evidence of internal consistency only --- not a precision figure --- and (iv) recommends that the CVSS strings be reviewed by a human triager before any external use. We do not claim CVSS validity; the point of the heuristic is to prefill the disclosure template with a starting vector that the analyst then refines.

\paragraph{Review~2: ``Real-world impact is modest (only one weak non-synthetic finding, not a CVE)''} The previous version already reproduced CVE-2015-8540 in \texttt{png\_check\_keyword} (libpng~1.6.19) and verified the same OOB persists in libpng~1.6.47. The revised version adds a third reproduction of the same CVE pattern in the structurally different libpng~1.2.56 code path (which uses \texttt{png\_strlen} + a \texttt{for(*kp)} loop instead of the inline \texttt{while}), demonstrating that the caller-contract assumption persists across two major code generations. Table~\ref{tab:cve} summarises the four runs (three confirmations, one counterfactual that flips the verdict to \texttt{infeasible}). The classification of \texttt{png\_format\_number} remains framed exactly as before --- a defense-in-depth contract violation, explicitly not a remotely-reachable CVE --- because the call-site grep we report still shows all internal callers respecting the contract; we deliberately do not strengthen this claim because the call-site survey we report does not support it.

\paragraph{Review~2: ``Main contribution is system integration rather than new symbolic execution or LLM methods''} The Contributions paragraph in Section~\ref{sec:intro} has been rewritten so that the methodological choices --- the candidate schema as a pre-commit specification, the retractable-bounds protocol that surfaces mis-modelled preconditions, and the symex-witness-to-disclosure pipeline --- appear as the contribution, with the system realisation labelled separately. The intent is to make explicit that the work is not ``LLM-plus-KLEE wiring'' but a discipline imposed on how the LLM speaks to a symbolic engine.

\paragraph{Review~2: ``Insufficient empirical evidence for strong acceptance''} The additions above (third CVE reproduction in libpng~1.2.56, baseline contrast paragraph, classifier consistency cross-check, three new analysis paragraphs) extend the empirical evidence within the original paper's scope. We acknowledge that a fuller evaluation --- larger CVE corpus, second-library reproductions, head-to-head against external tools --- is the right direction for a follow-on study and now say so in the limitations section.
