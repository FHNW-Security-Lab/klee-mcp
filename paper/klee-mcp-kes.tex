...
%% --- author additions: packages required by this paper's content ---
%% (the Procedia template preamble above is unmodified; these are the
%%  author definitions the template invites at "put your own definitions here")
\usepackage{amsmath}
\usepackage{listings}
\usepackage{xcolor}
\usepackage{booktabs}
\usepackage{tabularx}
\emergencystretch=3em   % prevent long URLs/identifiers overflowing the measure
...
\begin{figure}[t]
\centering
% Figure provided as a static PDF (fig-pipeline.pdf) so the manuscript
% has no live-graphics or external-code dependency, per Elsevier
% Editorial Manager LaTeX guidance. The diagram source is archived in the
% project repository.
